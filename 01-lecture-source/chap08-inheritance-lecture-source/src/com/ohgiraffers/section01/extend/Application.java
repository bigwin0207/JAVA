...
public class Application {
    public static void main(String[] args) {
        ...
        * */
        ...
        /*
        * 메소드 재정의(overriding이란? 부모가 가지는 메소드 선언부를 그대로 사용하면서 자식 클래스가
        ...
        *   1-2. 공동적으로 사용하는 코드가 부모클래스에 존재하면 수정사항이 생길 시 부모 클래스만 수정해도 전체적으로
        *        적용된다.(유지 보수성 증가)
        * 2. 클래스 간의 계층 관게가 형성되며 다형성의 문법적인 토대가 된다.
        * */

        /*
        * 상속으로 인한 단점
        * 1. 부모클래스의 기능을 추가. 변경 시에 자식클래스가 정상 동작하는지 예측이 힘들다.
        ...
        * 역할이란 동일한 동작을 수행하는 것을 정의한 것이면, 대체 가능성을 의미한다.
        * 부모 클래스를 추상화 하는 경우 역할의 관점으로 바라봐야한다.
        * 그래야 자식클래스로 생성한 객체들이 서로 역할을 수행해가며 유연한 코드를 작성할 수 있게 된다.
        * 동일한 역할을 가지는 모든 객체는 동일한 메세지를 수신하기는 하지만 객체별로 그 메시지에 응답하는 방식이
        * 서로 다를 수 있다.
        * */
